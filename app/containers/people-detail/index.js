...
import { addBlock, removeBlock } from '../../store/actions/block';
import { getPeopleListById } from '../../store/reducers/people';
...
@connect(
  (state, props) => ({
    me: getProfile(state),
    people: getPeopleListById(state, props.navigation.state.params.id)
  }),
  dispatch => ({
...
export default class PeopleDetail extends React.Component {
  ...
  getPeople() {
    ...
    if (people && people.data && people.data[0]) {
  ...
  async componentDidMount() {
    ...
    let { loadPeopleList, me, navigation } = this.props
      ...
      <View style={[styles.head, S['m-b-10'], S['m-t-10']]}>
        ...
        <View>
          ...
          <View style={{marginTop:10}}>
            <SendMessage people_id={people._id} nickname={people.nickname} />
          </View>
        </View>
      </View>
