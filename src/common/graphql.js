...
export default ({
  ...
  if (global.debug) console.log(api);

  return new Promise(resolve=>{
    return fn(options).then(res=>{

      if (global.debug) console.log(res);
      ...
      if (res.networkError) {
        resolve(['Network request failed']);
        return
      }
      ...
      if (res.graphQLErrors.length > 0) {
        ...
        // 账户被屏蔽
        if (res.graphQLErrors[0].blocked) {
          if (global.signOut) global.signOut();
          return;
        } else if (res.graphQLErrors[0].message == 'invalid token') {
          if (global.signOut) global.signOut();
          return;
        }
