...
import { htmlImgTransformText } from '../common/string';
...
const processNotificationList = (list) => {
  ...
  list.map(item => {
    ...
    if (item.comment_id) {
      item.comment_id.content_trim = htmlImgTransformText(item.comment_id.content_html, 50);
    }

    if (item.comment_id && item.comment_id.parent_id) {
      item.comment_id.parent_id.content_trim = htmlImgTransformText(item.comment_id.parent_id.content_html, 50);
    }

    if (item.comment_id && item.comment_id.reply_id) {
      item.comment_id.reply_id.content_trim = htmlImgTransformText(item.comment_id.reply_id.content_html, 50);
    }

    if (item.comment_id && item.comment_id.answer_id) {
      item.comment_id.answer_id.content_html = htmlImgTransformText(item.comment_id.answer_id.content_html, 50);
    }
...
export const cleanNotificationData = () => {
  return (dispatch, getState) => {
