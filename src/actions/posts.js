...
const processPostsList = (list) => {
  ...
  list.map(function(posts){
    ...
    if (posts.content_html) {
      ...
      if (textContent.length > 50) textContent = textContent.slice(0, 50)+'...';
...
export function loadNewPosts() {
  ...
  return new Promise(async (resolve, reject)=>{
    ...
    // 如果follow为空，不加载新帖子
    // 如果 last_find_posts_at 小于等于最近一条posts的创建日期，则不加载新帖子
    if (
      !lastPosts ||
      !profile ||
      !newPostsTips['/follow'] ||
      profile && newPostsTips['/follow'] && new Date(newPostsTips['/follow']).getTime() <= new Date(profile.last_find_posts_at).getTime()
    ) {
      resolve(false);
      return;
    }
...
export const newPostsTips = () => {
  return async (dispatch, getState) => {
    ...
    if (me && newPostsTips['/follow'] && new Date(newPostsTips['/follow']).getTime() > new Date(me.last_find_posts_at).getTime()) {
      // redPoint = true;
